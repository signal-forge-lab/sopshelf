...
このPublicリポジトリでは、ブランチは開発工程の分離にのみ使用し、機密性の境界としては使用しません。

- `main` — 公開可能で、利用可能な状態を維持する基準ブランチ。
- `develop` — 開発中変更を統合するPublicブランチ。
- `feature/*` — 必要なときだけ作成する短命なPublic機能ブランチ。

Secret、PC固有パス、ローカル専用設定は、どのブランチにもコミットしません。SOPS、または `.env`、`secrets/`、`local/`、`config.local.*` などGit管理外の領域を使用します。実装コード自体を非公開にする必要がある場合は、このPublicリポジトリ内の別ブランチではなく、別のPrivateリポジトリを使用します。
